...
- Startup time < 50ms achieved (compiled CLIs like Go/Rust)
- Startup time < 200ms acceptable for interpreted CLIs (Python/Node.js)
...
- NPM global packages
- npm pack for tarball distribution
...
The cli-developer agent supports the following `request_type` values:

| Request Type | Description | Response Schema |
|-------------|-------------|-----------------|
| `get_cli_context` | Get CLI requirements and workflows | `{status: "success", context: {use_cases, target_users, workflows, platforms, performance, distribution}}` |
| `validate_command` | Validate command structure and naming | `{status: "success", valid: boolean, issues: string[]}` |
| `suggest_completions` | Generate shell completion code | `{status: "success", code: string, shell: "bash|zsh|fish|powershell"}` |
| `design_prompts` | Design interactive prompt flow | `{status: "success", flow: {steps: [{type, prompt, options}]}}` |
...
```json
{
  "status": "error",
  "error": {
    "code": "INVALID_REQUEST | MISSING_PARAM | NOT_SUPPORTED",
...
```json
{
  "requesting_agent": "user-session",
  "request_type": "get_cli_context",
  "payload": {
    "query": "Need CLI for database migrations with interactive prompts"
  }
}
```
...
```json
{
  "status": "success",
  "context": {
    "use_cases": ["migration", "rollback", "status", "seeding"],
    "target_users": "developers",
    "workflows": ["interactive", "CI/CD automation"],
    "platforms": ["macOS", "Linux", "Windows"],
    "performance": {"startup_target": "200ms", "response_target": "100ms"},
...
```json
{
  ...
  "error": {
    "code": "MISSING_PARAM",
    "message": "Required parameter 'query' is missing from payload",
    "details": {"required": ["query"], "provided": ["platforms"]}
  }
}
```
...
"CLI implementation completed. Delivered cross-platform developer tool with 23 commands, 38ms startup time, and shell completions for all major shells. Reduced task completion time by 70% with interactive workflows and achieved 4.8/5 developer satisfaction rating."
