...
Before designing your API, choose the appropriate paradigm for your use case:

| Paradigm | Best For | Characteristics |
|----------|----------|----------------|
| **REST** | Standard CRUD operations, public APIs | Resource-oriented, stateless, HTTP semantics, caching-friendly |
| **GraphQL** | Complex data graphs, mobile clients | Flexible queries, single endpoint, type schema, over-fetching control |
| **gRPC** | Microservice communication, high performance | Protocol buffers, binary format, streaming, strong typing |
| **WebSocket** | Real-time updates, event-driven | Bidirectional, persistent connection, low latency |

**Choose REST when:** You need standard HTTP semantics, caching, and broad tooling support.
**Choose GraphQL when:** Clients have varying data needs and you want to avoid over-fetching.
**Choose gRPC when:** You need high-performance service-to-service communication with type safety.
**Choose WebSocket when:** You need real-time push updates from server to clients.
...
#### How to Send Context Queries

You can send context queries using three methods:

**1. CLI (Recommended for Development):**
```bash
# Direct command invocation
context query --agent backend-developer --type get_backend_context \
  --payload "service architecture, data stores, API gateway config"
```

**2. Internal API:**
```python
# Python example
from context.manager import query_context
response = await query_context(
    requesting_agent="backend-developer",
    request_type="get_backend_context",
    payload={"query": "service architecture, data stores..."}
)
# Handle timeout: max 30 seconds, raise QueryTimeoutError if exceeded
```
...
curl -X POST http://localhost:8080/api/context/query \
  ...
  -d '{"requesting_agent": "backend-developer", \
       "request_type": "get_backend_context", \
       "payload": {"query": "..."}}'
```
...
- If all retries fail, log error and proceed with conservative defaults
- Maximum wait time: 30 seconds total
...
> **Note:** The integration list below assumes a multi-agent ecosystem. If a specific agent is unavailable in your environment, work with your team to adjust the workflow or implement the missing functionality directly.
