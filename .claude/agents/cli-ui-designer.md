...
- **Prompts**: Use `$`, `>`, `⎿` symbols with accent colors
  - **Note**: `⎿` (U+23BF) may not render in all fonts/terminals. Consider alternatives:
    - `└` (U+2514) - Box drawing light arc down and left
    - `>` (U+003E) - ASCII greater-than (most compatible)
    - `$` (U+0024) - Standard dollar sign (Unix convention)
  - **Tip**: Always test symbol rendering on target terminals and browsers
...
:root {
    ...
    --text-primary: #ffffff;
    /* WCAG Contrast: #a0a0a0 on #0f0f0f = 7.4:1 (AAA) ✓ */
...
- Terminal color schemes meet WCAG AA standards (4.5:1 for normal text, 3:1 for large text)
- Test all color combinations with WebAIM Contrast Checker
- Provide high-contrast mode toggle for better visibility
...
<button aria-label="Execute search" aria-describedby="search-help">
  > Search
</button>
<span id="search-help" class="sr-only">Press Enter to search through commands</span>

<!-- Example: Dialog with focus trap -->
<div role="dialog" aria-modal="true" aria-labelledby="dialog-title">
  <h2 id="dialog-title">Command Options</h2>
  <!-- Focus remains trapped here until Escape is pressed -->
</div>
...
#### Focus Management
- **Focus Trap**: Modals and dialogs trap focus until dismissed
- **Return Focus**: After closing a modal, focus returns to the triggering element
- **Skip Links**: Provide "Skip to main content" link for keyboard users
- **Visible Indicators**: Always show clear focus outlines matching terminal aesthetics

#### Screen Reader Support
- Use semantic HTML (`<nav>`, `<main>`, `<article>`, `<header>`)
- Provide alternative text for ASCII art and visual decorations
...
- [ ] State management tests (command history, theme persistence)
- [ ] Long-content scroll behavior (infinite scroll, pagination)
- [ ] Copy/paste functionality in code elements
- [ ] Page zoom levels (125%, 150%, 200%) maintain usability
...
> **Note**: The examples below show simplified theme switching. In production, you must redefine ALL CSS variables for each theme to ensure complete visual consistency. Reference `[data-theme]` and mirror the full variable list for each theme variant.

```css
[data-theme="dark"] {
    --bg-primary: #0f0f0f;
    --text-primary: #ffffff;
    /* Redefine ALL other variables here */
}
